...
# Funcion para extraer "high gamma" features
# 3 filtros distintos:
def extractHG(data, sr,windowLength=0.05,frameshift=0.01):
    ...
    data = filter_data(data.T.astype(np.float), sr, 70,170,method='iir').T # Paso banda, 70 Hz y 170 Hz
    data = filter_data(data.T.astype(np.float), sr, 102, 98,method='iir').T # Band-stop.  Paso banda, 102 Hz y 98 Hz
    data = filter_data(data.T.astype(np.float), sr, 152, 148,method='iir').T  #Paso banda, 152 Hz y 148 Hz
    data = np.abs(hilbert3(data))   # Les aplica una transformada de Hilbert
...
# Vector de parametros 29993 filas y 1125 columnas
...
# Extrae coeficientes Mel, 23 coeficic
    # ventana 20 ms, solapamiento 5 ms
def extractMelSpecs(audio, sr, windowLength=0.05, frameshift=0.01):
    ...
    for w in range(numWindows):
        ...
        spectrogram[w,:] = spec
        # LO HE CAMBIADO POR 10 MFCC, ORIGINAL ERAN 23
    mfb = mel.MelFilterBank(spectrogram.shape[1], 10, sr)
...
if __name__=="__main__":
    ...
    for pNr, p in enumerate(pts):
        for ses in range(1,sessions[pNr]+1):
            ...
            # ANTES ESTABA A 16 KHZ, LO HE CAMBIADO A 8 KHZ
            targetSR = 8000
